    ...
    class Consumer
      ...
      def run(msg_stream)
        ...
        queue.subscribe(subscribe_opts) do |delivery_info, metadata, payload|
          ...
          logger.debug "Consumed message #{message.headers[:type]}[#{message.headers[:message_id]}] on channel: #{@channel.id} delivery tag: #{message.delivery_info[:delivery_tag]}"
      ...
      def ack(incoming, answer: nil)
        ...
        logger.debug "Ack message #{incoming.headers[:type]}[#{incoming.headers[:message_id]}] on channel: #{@channel.id} delivery tag: #{incoming.delivery_info[:delivery_tag]}"
      ...
      def nack(incoming, error: nil)
        logger.debug "Reject message #{incoming.headers[:type]}[#{incoming.headers[:message_id]}] on channel #{@channel.id} delivery tag: #{incoming.delivery_info[:delivery_tag]}"
